...
"""Validate the MatrixCache hit-concurrency JSON report."""
...
PRIVATE_ROW_FIELDS = [
    "threads",
    "get_refresh_distance_zero_mops_per_s",
    "get_no_move_mops_per_s",
    "acquire_mops_per_s",
    "no_promotion_acquire_mops_per_s",
]
...
def main() -> None:
    ...
    parser.add_argument("--min-throughput-mops", type=float, default=0.0)
    ...
    if report.get("report_version") != "matrixcache_hit_concurrency_v1":
        fail("unexpected report_version")
    for field in ["resident_values", "value_bytes", "reads_per_thread", "passes"]:
    ...
    for index, row in enumerate(rows):
        ...
        for field in required_fields[1:]:
            value = row[field]
            if not isinstance(value, (int, float)) or value <= args.min_throughput_mops:
                fail(
                    f"row {index} field {field} must be greater than "
                    f"{args.min_throughput_mops}"
                )

    print(
        "OK matrixcache hit concurrency report: "
