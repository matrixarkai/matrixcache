...
out.append("")
out.append("/// Reusable buffers for a MatrixCache Prometheus scrape.")
out.append("///")
out.append("/// Long-running metrics endpoints can keep one of these per handler thread")
out.append("/// and avoid rebuilding the exposition body and label scratch allocation on")
out.append("/// each scrape.")
out.append("#[derive(Debug, Clone)]")
out.append("pub struct PrometheusScrapeBuffer {")
out.append("    body: String,")
out.append("    labels: String,")
out.append("}")
out.append("")
out.append("impl Default for PrometheusScrapeBuffer {")
out.append("    fn default() -> Self {")
out.append("        Self::new()")
out.append("    }")
out.append("}")
out.append("")
out.append("impl PrometheusScrapeBuffer {")
out.append("    pub fn new() -> Self {")
out.append("        Self {")
out.append("            body: String::with_capacity(PROMETHEUS_TEXT_CAPACITY_BYTES),")
out.append("            labels: String::with_capacity(PROMETHEUS_LABELS_CAPACITY_BYTES),")
out.append("        }")
out.append("    }")
out.append("")
out.append("    pub fn render(&mut self, stats: &CacheStats, labels: &[(&str, &str)]) -> &str {")
out.append("        prometheus_text_into_with_label_scratch(stats, labels, &mut self.body, &mut self.labels);")
out.append("        self.body.as_str()")
out.append("    }")
out.append("")
out.append("    pub fn as_str(&self) -> &str {")
out.append("        self.body.as_str()")
out.append("    }")
out.append("")
out.append("    pub fn body_capacity(&self) -> usize {")
out.append("        self.body.capacity()")
out.append("    }")
out.append("")
out.append("    pub fn label_capacity(&self) -> usize {")
out.append("        self.labels.capacity()")
out.append("    }")
out.append("")
out.append("    pub fn clear(&mut self) {")
out.append("        self.body.clear();")
out.append("        self.labels.clear();")
out.append("    }")
out.append("}")
out.append("")
